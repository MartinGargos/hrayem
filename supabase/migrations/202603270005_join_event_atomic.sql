...
begin
  ...
  if previous_event_status <> desired_event_status then
    ...
    locked_event.status := desired_event_status;
  end if;

  if locked_event.organizer_id is not null then
    ...
    values (
      locked_event.organizer_id,
      p_event_id,
      'player_joined',
      jsonb_build_object(
        'joined_user_id', p_user_id,
        'status', resulting_player.status
      )
    );
  end if;

  if resulting_player.status = 'confirmed' then
    insert into public.notification_log (
      user_id,
      event_id,
      type,
  ...
  if previous_event_status <> 'full'
     and locked_event.status = 'full'
     and locked_event.organizer_id is not null then
    insert into public.notification_log (
      user_id,
      event_id,
      type,
