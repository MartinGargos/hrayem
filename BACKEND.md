...
**Universal links:** `https://www.hrayem.cz/event/{id}` (for production sharing)
...
- Deploy an `apple-app-site-association` file and `assetlinks.json` to `www.hrayem.cz/.well-known/`
- Use `expo-linking` for handling incoming links

**Deep link routing:**
- `www.hrayem.cz/event/{id}` → Event detail screen (if logged in) or Login → Event detail (after auth)
...
- Event detail screen includes a share button that calls `Share.share()` with:
  ...
  - URL: `https://www.hrayem.cz/event/{id}`
- The web fallback page at `www.hrayem.cz/event/{id}` is a simple static/SSR page showing event details + app download links
